...
A dictionary holding certificates.

The dictionary keys are the certificate names, the values get merged into the certificate object.
The keys can contain an optional namespace prefix in the form of `namespace/name`.
If no namespace is specified, the certificate will be created in the namespace defined by parameter `namespace`.
...
BadThingsHappening:
  ...
  rule:
    annotations:
      summary: Bad things have been happening on {{$labels.node}} for more than 10 minutes.
    expr: |
...
`enabled` is a mandatory field to specify whether the alert should be installed or not.
`rule` contains the Prometheus alerting rule definition.
